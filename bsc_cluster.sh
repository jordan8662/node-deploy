...
function prepare_config() {
    ...
    sed -i 's/"balance": "0x033b2e3c9fd0803ce8000000"/0x6c7974123f64a40000/g' genesis.json
...
function register_stakehub(){
    ...
    for ((i = 0; i < size; i++));do
        ${workspace}/create-validator/create-validator --consensus-key-dir ${workspace}/keys/validator${i} --vote-key-dir ${workspace}/keys/bls${i} \
            --password-path ${workspace}/keys/password.txt --amount 2001 --validator-desc Val${i} --rpc-url ${RPC_URL}
    done
}
